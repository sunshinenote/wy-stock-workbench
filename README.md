...
- **每日复盘**：五大指数快照、上证 90 日趋势图（内联 SVG）、板块涨幅榜、自选股行情（可录入成本算盈亏）
  - **复盘笔记**：每日记录自动归档，本地存储
  - **概念龙头**：行业 → 细分概念 → 龙头股速查，支持关键词搜索与行业筛选；龙头带实时涨跌幅（腾讯行情，每日采集）
...
└── data/
    ...
    ├── concepts.json        # 行业/概念/龙头股 参考清单（人工维护，可编辑）
    ├── concepts_quotes.json # 龙头股实时行情（采集生成）
    └── *.json              # 采集输出（自动生成）
```
...
`data/concepts.json` 是人工维护的「行业 → 细分概念 → 龙头股（代码+名称）」参考清单，随市场变化需不定期更新。直接编辑该文件后重新采集（或本地 `python collect.py`）即可生效；龙头股的实时涨跌幅由采集脚本自动从腾讯行情获取，写入 `data/concepts_quotes.json`。龙头以辨识度高、代表性强的标的为主，仅供参考、不构成投资建议。
