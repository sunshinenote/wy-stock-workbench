...
def _latest_report_period():
    """根据当前日期返回最近已「基本披露完」的季度报告期（牛散/机构数据随季报更新）

    注意：不要死等法定披露截止日。中报截止虽为 8/31，但 8/20 之后披露率通常已超七成，
    此时继续沿用上一期（一季报）会让数据滞后整整一个季度。故 8 月下旬即切到中报。
    """
    m = NOW.month
    y = NOW.year
    if 5 <= m <= 7:
    ...
    if m == 8:
        return f"{y}0630" if NOW.day >= 20 else f"{y}0331"  # 8月下旬：中报已基本出齐
    if 9 <= m <= 10:
